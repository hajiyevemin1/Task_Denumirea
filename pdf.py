...
columns = ["N/o", "Data anuntului", "IDNO", "Denumirea", "Adresa", "Din", "In"]

x_bounds = [33.36, 53.16, 101.16, 169.34, 310.97, 472.75, 639.82, 807.60]
...
def parse_page(page, y_min, y_max):
    ...
    for y0, y1 in zip(y_lines[:-1],y_lines[1:]):
        row = [read_cell(page, x_bounds[i], x_bounds[i+1],y0, y1) for i in range(len(x_bounds) - 1)]
...
for page_index in range(len(doc)):
    ...
    y_min = 120 if page_index == 0 else 20
